...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    var mostRetweetedValue = elem
    val mostRetweetedFunction: (Tweet) => Unit = t => if (t.retweets > mostRetweetedValue.retweets) mostRetweetedValue = t
    left.foreach(mostRetweetedFunction)
    right.foreach(mostRetweetedFunction)
    mostRetweetedValue
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter(t => google.exists(w => t.text.contains(w)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets
    .filter(t => apple.exists(w => t.text.contains(w)))
...
object Main extends App {
  ...
  GoogleVsApple.trending.foreach(f => println(f))
}
